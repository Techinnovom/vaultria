"""Generate site/assets/css/family-law.css from _family_law_extracted.html."""
import re
from pathlib import Path

ROOT = Path(__file__).resolve().parents[1]
raw = (ROOT / "site/pages/_family_law_extracted.html").read_text(encoding="utf-8")
...
header = """/* Family Law (Codex Vol. V) — scoped to site shell */
.wrap.family-wrap {
...
.family-law-page {
  ...
  --purple: #7848c8;
  --purplel: rgba(120, 72, 200, 0.08);
  --purpleb: rgba(120, 72, 200, 0.28);
  font-size: 1.02rem;
  line-height: 1.75;
  color: var(--tx);
}

.family-law-page .tier-kicker a {
  color: var(--gold);
}

.family-law-page *,
.family-law-page *::before,
.family-law-page *::after {
...
if old_media in rest:
    ...
    _bad680 = "@media(max-width:680px){.sb{display:none}.main{margin-left:0}.hero,.content{padding-left:20px;padding-right:20px}.principles{grid-template-columns:1fr}.g2{grid-template-columns:1fr}}"
...
out = header + shell_ov + "\n" + rest
out = out.replace(
    ".family-law-page .sb-a{display:flex;",
    ".family-law-page .sb-a{display:flex;color:inherit;",
    1,
)
(ROOT / "site/assets/css/family-law.css").write_text(out, encoding="utf-8")
print("Wrote", ROOT / "site/assets/css/family-law.css", len(out), "chars")
